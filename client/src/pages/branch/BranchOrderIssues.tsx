...
import {
  ...
  Phone, Filter, Timer, Image as ImageIcon, Gift, Copy, Lightbulb, MessageSquare, Trash2
} from "lucide-react";
...
const STATUS_CONFIG: Record<string, { label: string; icon: typeof Clock; color: string; badgeVariant: "default" | "secondary" | "destructive" | "outline" }> = {
  ...
  escalated: { label: "ส่งต่อ HQ", icon: AlertTriangle, color: "text-orange-500", badgeVariant: "destructive" },
  closed: { label: "ปิดแล้ว", icon: XCircle, color: "text-gray-500", badgeVariant: "outline" },
};

export default function BranchOrderIssues() {
  ...
  const queryInput = useMemo(() => {
    ...
    if (isAreaManager && selectedBranchId) input.branchId = selectedBranchId;
    return Object.keys(input).length > 0 ? input : undefined;
  }, [statusFilter, isAreaManager, selectedBranchId]);

  const { data: issues, isLoading, refetch } = trpc.orderIssues.list.useQuery(queryInput, {
  ...
  const { data: guideline } = trpc.orderIssues.getSuggestedGuideline.useQuery(
    { category: selectedIssue?.category || "other" },
    { enabled: !!selectedIssue && showGuideline }
  );
  ...
  const createCompMutation = trpc.claims.create.useMutation({
    onSuccess: (data) => {
      ...
      toast.success("สร้างโค้ดชดเชยสำเร็จ! สถานะปัญหาเปลี่ยนเป็น 'แก้ไขแล้ว' อัตโนมัติ");
      refetch();
    },
    onError: (err) => toast.error(err.message),
  });
  ...
  return (
    <MobileLayout title="ปัญหาออเดอร์" showBack backPath="/branch">
      <PremiumPageContent>
        ...
        <div className="flex items-center justify-between">
          <div>
            <h2 className="text-lg font-bold">ปัญหาออเดอร์{isAreaManager && selectedBranchId ? ` - ${session.managedBranches?.find(b => b.id === selectedBranchId)?.name ?? ""}` : "สาขา"}</h2>
            ...
            {issues.map((item: any) => {
              ...
              return (
                <Card
                  ...
                  onClick={() => { setSelectedIssue({ ...issue, customerName, customerPhone }); setShowCompForm(false); setShowGuideline(false); setCompResultCode(""); }}
                >
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  ...
                  <div>
                    ...
                    <Badge variant={STATUS_CONFIG[selectedIssue.status]?.badgeVariant || "outline"} className="text-[10px]">
                  ...
                  <div className="border border-indigo-200 bg-indigo-50/50 rounded-xl p-3">
                    <div className="flex items-center justify-between mb-1">
                      <div className="flex items-center gap-2">
                        ...
                        <p className="text-xs font-semibold text-indigo-800">คำแนะนำจาก HQ</p>
                      </div>
                      <Button
                        variant="ghost"
                ...
                {showAdminNoteForm && (
                  <div className="border border-indigo-200 bg-indigo-50/50 rounded-xl p-3 space-y-2">
                    <div className="flex items-center gap-2 mb-1">
                      <MessageSquare className="h-4 w-4 text-indigo-600" />
                      <p className="text-xs font-semibold text-indigo-800">เพิ่มข้อความถึงลูกค้า / HQ</p>
                    </div>
                    <Textarea
                      value={newAdminNote}
                    ...
                    <div className="flex gap-2">
                      <Button variant="outline" size={isMobile ? "default" : "sm"} className="flex-1" onClick={() => { setShowAdminNoteForm(false); setNewAdminNote(""); }}>ยกเลิก</Button>
                      <Button size={isMobile ? "default" : "sm"} className="flex-1" onClick={() => addAdminNoteMutation.mutate({ id: selectedIssue.id, adminNote: newAdminNote })} disabled={addAdminNoteMutation.isPending || !newAdminNote.trim()}>
                ...
                {!["closed", "resolved"].includes(selectedIssue.status) && (
                  ...
                  >
                    ...
                    {showGuideline ? "ซ่อนแนวทางแก้ปัญหา" : "ดูแนวทางแก้ปัญหาแนะนำ"}
                  </Button>
                )}
                ...
                {showGuideline && guideline && (
                  <div className="border border-indigo-100 bg-indigo-50/30 rounded-xl p-4 space-y-2">
                    <p className="text-sm font-semibold text-indigo-800">{guideline.title}</p>
                    <ol className="space-y-1.5 text-xs text-indigo-700">
                      {guideline.steps.map((step: string, idx: number) => (
                        <li key={idx} className="flex gap-2">
                          <span className="flex-shrink-0 w-5 h-5 rounded-full bg-indigo-200 text-indigo-800 flex items-center justify-center text-[10px] font-bold">{idx + 1}</span>
                          <span className="pt-0.5">{step}</span>
                        </li>
                      ))}
                    </ol>
                    <p className="text-[10px] text-indigo-500 italic mt-2">{guideline.tips}</p>
                  </div>
                )}

                {/* ── Compensation Code Section ── */}
                {!showCompForm && !compResultCode && !["closed", "resolved"].includes(selectedIssue.status) && (
                  <Button
                    variant="outline"
                ...
                {compResultCode && (
                  <div className="border border-green-200 bg-green-50/50 rounded-xl p-4 space-y-3">
                    ...
                    <p className="text-[10px] text-green-600">สถานะปัญหาเปลี่ยนเป็น "แก้ไขแล้ว" อัตโนมัติ</p>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center justify-between">
                        ...
                        <Button variant="ghost" size={isMobile ? "default" : "sm"} className="h-7 text-xs" onClick={() => { navigator.clipboard.writeText(compResultCopyText); toast.success("คัดลอกแล้ว!"); }}>
                ...
                {selectedIssue.status === "open" && (
                  <div className="space-y-2">
                    <Button
                      ...
                      className="w-full"
                    >
                      {acknowledgeMutation.isPending ? <Loader2 className="h-4 w-4 animate-spin mr-2" /> : <CheckCircle2 className="h-4 w-4 mr-2" />}
                      รับทราบปัญหา
                    </Button>
                  </div>
                )}
                ...
                <DialogFooter className="flex gap-2">
                  {!["closed", "resolved"].includes(selectedIssue.status) && (
                    <Button variant="outline" size={isMobile ? "default" : "sm"} className="text-xs" onClick={() => { toast.info("ส่งต่อไป HQ เรียบร้อย (ฟีเจอร์กำลังพัฒนา)"); }}>
                      ส่งต่อ HQ
                    </Button>
                  )}
                </DialogFooter>
